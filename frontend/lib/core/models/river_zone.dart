...
class RiverZone {
  ...
  double get capacityPercentage =>
      safeCapacity > 0 ? (currentHeadcount / safeCapacity).clamp(0.0, 2.0) : 0.0;

  String get calculatedStatus {
    if (restrictedEntry) return 'CRITICAL';
  ...
  bool get isSafe => calculatedStatus == 'SAFE';
  bool get isModerate => calculatedStatus == 'MODERATE';
  bool get isHigh => calculatedStatus == 'HIGH';
  bool get isCritical => calculatedStatus == 'CRITICAL';
  ...
  factory RiverZone.fromMap(Map<String, dynamic> map) {
    final updateStr = map['updated_at'] ?? map['last_update'];
    final speed = (map['water_speed'] ?? map['water_flow_speed_kmh']) as num?;
    final restricted = map['restricted_entry'];
    ...
    return RiverZone(
      ...
      lastUpdate: updateStr != null
          ? DateTime.tryParse(updateStr.toString()) ?? DateTime.now()
          : DateTime.now(),
      restrictedEntry: restricted == true || restricted == 1,
      restrictedAt: map['restricted_at'] != null
          ? DateTime.tryParse(map['restricted_at'].toString())
          : null,
      restrictedBy: map['restricted_by']?.toString(),
      currentHeadcount: (map['current_headcount'] as num?)?.toInt() ?? 0,
      safeCapacity: (map['safe_capacity'] as num?)?.toInt() ?? 5000,
      waterFlowSpeedKmh: speed?.toDouble() ?? 1.5,
