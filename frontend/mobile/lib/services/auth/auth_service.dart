...
class AuthService {
  ...
  /// Re-throws [FirebaseAuthException] or general exceptions so caller can handle them.
  ...
  /// Throws descriptive [FirebaseAuthException] on missing profile or unauthorized role.
  Future<String> loginAndGetRole(String email, String password) async {
    ...
    if (!doc.exists || doc.data() == null) {
      debugPrint(
          '[AUTH DIAGNOSTIC] Document at "user/$uid" DOES NOT EXIST or data is null.');
      await _auth.signOut();
      throw FirebaseAuthException(
  ...
  /// Only executes when explicitly called by application logic.
  ...
  /// Returns null if user is not authenticated or profile document does not exist.
